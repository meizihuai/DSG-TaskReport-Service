...
namespace DSG_TaskReport_Service
{
    ...
    /// </summary>
    /// <returns></returns>
    class FreqSampleService : ITaskReportService
    {
        public Task<NormalResponse> DoReport(UserTaskInfo taskInfo, string deviceId, string dirPath)
        {
            return Task.Run(() =>
            {
                TaskFreqSample ts = JsonConvert.DeserializeObject<TaskFreqSample>(taskInfo.TaskCode);
                ...
                using (DocX doc = DocX.Create(filePath))
                {
                    ...
                    doc.InsertParagraph()
                    .AppendLine($"     本次监测任务主要监测[{ts.FreqStart.ToString("0.000")}MHz,{ts.FreqStop.ToString("0.000")}MHz]频段,在{taskInfo.StartTime}到{taskInfo.EndTime}期间的频谱取样")
                    .Font("宋体").FontSize(15);
                    ...
                    doc.InsertParagraph().AppendLine("3.各时间频谱实时图").Font("宋体").FontSize(20);
                    freqlist.ForEach(a =>
                    {
                        string freqLabel = $"[{freqStart},{freqStop}]频段,在{a.DateTime}的实时频谱图";
                        string imgPath = TaskUtil.FreqValues2ImgFile(a.Freqs, a.FreqValues, freqLabel, Color.Blue);
                        var nImg = doc.AddImage(imgPath);
                        var pic = nImg.CreatePicture();
                        pic.Width = 600;
                        pic.Height = 200;
