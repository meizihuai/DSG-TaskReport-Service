...
{
    class SignalStatusWarnService : ITaskReportService
    {
        public Task<NormalResponse> DoReport(UserTaskInfo taskInfo, string deviceId, string dirPath)
        {
            return Task.Run(() =>
            {
                TaskSignalStatusWarn ts = JsonConvert.DeserializeObject<TaskSignalStatusWarn>(taskInfo.TaskCode);
                ...
                using (DocX doc = DocX.Create(filePath))
                {
                    ...
                    doc.InsertParagraph()
                    .AppendLine($"     本次监测任务主要监测{JsonConvert.SerializeObject(ts.WatchPoints)}MHz频点列表,在{taskInfo.StartTime}到{taskInfo.EndTime}期间的状态和预警信息")
                    .Font("宋体").FontSize(15);
                    ...
                    for (int i = 0; i < signallist.Count; i++)
                    {
                        ...
                        object[] obj = new object[]
                        {
                            ...
                            s.AvgValue.ToString("0.0"),s.StatusMark,$"{s.Occupy} %"
                        };
                        tab.InsertRow(obj);
                    }
